...
def get_network_stats():
    ...
    for dev in network_stats:
        ...
        interface_count += 1
        network_stats[dev].update({"received": {"bytes": float(rx_bytes), "packets": float(rx_packets)}})
        network_stats[dev].update({"transmitted": {"bytes": float(tx_bytes), "packets": float(tx_packets)}})
