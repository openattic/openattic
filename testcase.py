...
class GatlingTestCase(unittest.TestCase):
    ...
    def send_request(cls, method, prefixes=None, *args, **kwargs):
        ...
        Args:
            method (string)                     : The needed HTTP-method: POST, PUT, GET, DELETE
            prefixes (Optional[array of str])   : Optional parameter. Defaults to None. If prefixes is None, the class
                must define a api_prefix class-variable otherwise the test will be skipped.
                Must contain at least 1 and at most 3 REST API navigation prefixes. The first string is the first
                navigation parameter. The second string is the subnavigation parameter in case of detail routes.
                The last parameter is the navigation parameter used by the cleanup_url.
                ...
                Examples:
                    ...
                    ["snapshots", "clone", "volumes"]   ->  http://host/openattic/api/snapshots/snap_id/clone
                                                            and returns
                                                            http://host/openattic/api/volumes/vol_id
            **kwargs                            :  obj_id (int)      -> Object id to get a specific object by GET
                                                                        request.
                                                   data (dict)       -> Data for creating a new object by POST request.
                                                   search_param (str)-> Search param, e.g.: "name=vol_name" to
                                                                        search for a specific volume. It generates the
                                                                        following url:
        ...
        Returns:
            In case of:
                ...
                -   DELETE requests: A dictionary {"response": api_response, "count": 1, "cleanup_url": cleanup_url,
                    "headers": creation headers}
                -   GET requests: For a specific objects a dictionary {"response": api_response, "count": 1,
                    "cleanup_url": cleanup_url, "headers": creation headers}. For a list of objects a dictionary
                    {"response": api_response, "count": sum of returned objects}
        """
