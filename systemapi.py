...
class SystemD(BasePlugin):
    ...
    def osd_pool_create(self, cluster, poolname, pg_num, ruleset, sender):
        self.invoke_ceph(cluster, ["osd", "pool", "create", poolname, str(pg_num), str(pg_num), "replicated", str(ruleset)])

    @deferredmethod(in_signature="ss")
    def osd_pool_delete(self, cluster, poolname, sender):
        self.invoke_ceph(cluster, ["osd", "pool", "delete", poolname, poolname, "--yes-i-really-really-mean-it"])
